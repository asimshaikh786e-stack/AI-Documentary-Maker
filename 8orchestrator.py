...
def generate_scene_media(self, scene: Dict):
    ...
    audio_path = f"/tmp/{project_id}/audio_{scene_num}.mp3"
    try:
...
def assemble_task(self, scene_results: List[Dict], project_id: int):
    ...
    final_output = f"/tmp/{project_id}/FINAL_DOCUMENTARY.mp4"
    clips_to_concat = []
    ...
    if clips_to_concat:
        ...
        logger.info(f"Mubarak ho! Documentary taiyar hai: {final_output}")
    else:
...
def start_pipeline(project_id: int, prompt: str):
    ...
    def trigger_parallel(scenes: List[Dict]):
        # Creates a media generation task for EACH scene
        tasks = [generate_scene_media.s(s) for s in scenes]
        # Once all scenes are generated, pass them to assemble_task
